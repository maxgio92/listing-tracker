# Renovation Cost Guide (Italy)

Reference for estimating the total cost of a listing tracked by this tool
when the plan is converting a commercial property into an apartment.
Figures are rough 2026 estimates for a small-town Italian market, not
quotes; validate them locally before deciding.
...
- Use a registered building company, not a solo mason, and nothing off the
  ...
  20k EUR.
- Possible saving: run the new systems in a false ceiling instead of
  chasing all the walls.

## Purchase costs

- Real estate agency commission: 4% of the purchase price.

## Using these figures

A workable pattern is a parameters-plus-calculator tab in the same
spreadsheet the tool writes to: one table with the unit costs above, and a
small block that takes a listing's price and surface and computes the
agency fee, the works cost per scenario, and the totals. Sorting the
listings tab by price per m2 (column F) then running candidates through the
calculator gives a quick total-cost comparison.
